...
// Modified code by Alexander Nicoara for CS450
...
// we are using semicmd for & too except we are conditioning the wait so the scheduler can schedule parallel processes
    ...
    if(ecmd->argv[0] == 0)
{
  ...
  case ';':
    ...
    // we iterate through until no commands are left

    while(head != 0){



      // while there are parallel commands in the chain

      while(head->type == '&'){

        pid = fork1(); // create new process
        ...
        if(pid < 0){
          ...
          head = ((struct semicmd*)head)->nextCmd; // point to the next instance



          if(head->type != '&'){ // wait only if we hit a non & command
      ...
      // while there are sequential commands in the chain
      while(head->type == ';'){
        ...
        if(pid < 0){
          ...
          wait(&wstatus); // wait until the current process is finished
          temp = head;
          head = ((struct semicmd*)head)->nextCmd; // point to the next instance
  ...
  // case '&' is pretty much a copy of case ';', but we have to use command types for the switch statement

  case '&':
    head = cmd;

    // while there are commands (any type) left in the chain

    while(head != 0){

      // while there are ampersand commands to be executed
      while(head->type == '&'){
        pid = fork1();
        ...
        if(pid < 0){
          ...
          if(head->type != '&'){ // wait only if we hit a non ampersand command
      ...
      // while there are sequential commands to be executed
      while(head->type == ';'){
        ...
        if(pid < 0){
          ...
          temp = head;
          head = ((struct semicmd*)head)->nextCmd; // point to the next command
        }
...
struct cmd *parsesemi(char**, char*); // create a new parse function that handles our new command operators & and ;
...
// breaks about ; and & from input and parses commands based off them
...
//testing to see how peek works
  ...
  // if the remaining input contains ; or &
